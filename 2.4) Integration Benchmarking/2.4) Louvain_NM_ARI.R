...
source.clusters <- function(obj) {
  ...
  obj <- obj[order(obj$X),]
  obj <- obj %>% select(RNA_snn_res.0.8)
  obj
}
...
# Function to create louvain luster vectors for subsetted integrated files
...
#Selecting
harmony_fernandes <- harmony_fernandes %>% select(RNA_snn_res.0.8)
harmony_pan <- harmony_pan %>% select(RNA_snn_res.0.8)
harmony_alsaigh<- harmony_alsaigh %>% select(RNA_snn_res.0.8)
...
#Selecting
scanorama_fernandes <- scanorama_fernandes %>% select(pano_snn_res.0.8)
scanorama_pan <- scanorama_pan %>% select(pano_snn_res.0.8)
scanorama_alsaigh<- scanorama_alsaigh %>% select(pano_snn_res.0.8)
...
#Selecting
rPCA_fernandes <- rPCA_fernandes %>% select(integrated_snn_res.0.8)
rPCA_pan <- rPCA_pan %>% select(integrated_snn_res.0.8)
rPCA_alsaigh<- rPCA_alsaigh %>% select(integrated_snn_res.0.8)
...
## fernandes
harmony_fernandes_vector <- as.vector(harmony_fernandes['RNA_snn_res.0.8'])
fernandes_vector <- as.vector(fernandes['RNA_snn_res.0.8'])
...
harmony_alsaigh_vector <- as.vector(harmony_alsaigh['RNA_snn_res.0.8'])
alsaigh_vector <- as.vector(alsaigh['RNA_snn_res.0.8'])
...
harmony_pan_vector <- as.vector(harmony_pan['RNA_snn_res.0.8'])
pan_vector <- as.vector(pan['RNA_snn_res.0.8'])
...
## fernandes
scanorama_fernandes_vector <- as.vector(scanorama_fernandes['pano_snn_res.0.8'])
fernandes_vector <- as.vector(fernandes['RNA_snn_res.0.8'])
...
scanorama_alsaigh_vector <- as.vector(scanorama_alsaigh['pano_snn_res.0.8'])
alsaigh_vector <- as.vector(alsaigh['RNA_snn_res.0.8'])
...
scanorama_pan_vector <- as.vector(scanorama_pan['pano_snn_res.0.8'])
pan_vector <- as.vector(pan['RNA_snn_res.0.8'])
...
## fernandes
rPCA_fernandes_vector <- as.vector(rPCA_fernandes['integrated_snn_res.0.8'])
fernandes_vector <- as.vector(fernandes['RNA_snn_res.0.8'])
...
rPCA_alsaigh_vector <- as.vector(rPCA_alsaigh['integrated_snn_res.0.8'])
alsaigh_vector <- as.vector(alsaigh['RNA_snn_res.0.8'])
...
rPCA_pan_vector <- as.vector(rPCA_pan['integrated_snn_res.0.8'])
pan_vector <- as.vector(pan['RNA_snn_res.0.8'])
rPCA_pan_vector <- unlist(rPCA_pan_vector) 
pan_vector <- unlist(pan_vector)

rPCA_NMI_pan <- print(aricode::NMI(rPCA_pan_vector , pan_vector))
rPCA_ARI_pan <- print(aricode::ARI(rPCA_pan_vector , pan_vector))
...
NMI_RESULTS_LOUVAIN <- data.frame(Source,Harmony_NMI,rPCA_NMI,Scanorama_NMI)
...
# Join the variables to create a data frame
ARI_RESULTS_LOUVAIN <- data.frame(Source,Harmony_ARI,rPCA_ARI,Scanorama_ARI)
